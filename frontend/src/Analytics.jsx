...
import { RefreshCw, ArrowLeft, ShieldCheck, Zap, Image as ImageIcon, MessageCircle } from 'lucide-react';
...
export default function Analytics({ token, onClose }) {
    ...
    return (
      <div className="analytics-error-container">
        <div className="analytics-error-card">
          ...
          <p>{error}</p>
          <button onClick={() => fetchAnalytics()} className="analytics-btn-primary">
  ...
  stats.query_types.forEach(q => {
    if (!['maternal_health', 'child_health', 'scheme_eligibility', 'referral_decision', 'drug_protocol', 'general_health'].includes(q.query_type)) {
      const meta = queryTypeMeta[q.query_type] || { label: q.query_type, color: 'var(--text-muted)' };
      queryTypesList.push({
        type: q.query_type,
        count: q.count,
        label: meta.label,
  ...
  const totalLanguagesCount = languageList.reduce((acc, curr) => acc + curr.count, 0);
  ...
  const donutSegments = languageList.map(lang => {
    const percent = totalLanguagesCount > 0 ? lang.count / totalLanguagesCount : 0;
  ...
  const plotWidth = lineChartWidth - lineChartPadding.left - lineChartPadding.right;
  const plotHeight = lineChartHeight - lineChartPadding.top - lineChartPadding.bottom;
  ...
  const linePoints = dailyData.map((d, i) => {
    const x = lineChartPadding.left + (i / Math.max(dailyData.length - 1, 1)) * plotWidth;
    const y = lineChartPadding.top + plotHeight - (d.count / maxDayCount) * plotHeight;
    ...
    <div className="analytics-container">
      <header className="analytics-header">
        <div className="analytics-header-left">
          <button onClick={onClose} className="analytics-back-btn" title="Back to Chat">
            <ArrowLeft className="w-5 h-5" />
          </button>
          <div>
            <h2>Your Analytics Dashboard</h2>
            <p className="analytics-subtitle">Query trends and activity summaries for your account</p>
          </div>
        </div>
        <button
      ...
      {isEmpty ? (
        <div className="analytics-empty-state">
          ...
          <p>Start chatting with HealthBridge AI or upload medical documents. Once you make queries, your diagnostic support trends will appear here.</p>
        ...
        <div className="analytics-content">
          ...
          <div className="analytics-charts-grid">
            <div className="chart-card">
              ...
              <div className="bar-chart-container">
                {queryTypesList.map((item) => {
                  const percentage = stats.total_queries > 0 ? (item.count / stats.total_queries) * 100 : 0;
                  return (
            ...
            <div className="chart-card flex-col-center">
              <h3 className="chart-title align-self-start">Language Distribution</h3>
            ...
            <div className="chart-card full-width-chart">
              ...
              <div className="line-chart-container">
                <svg width="100%" height="100%" viewBox={`0 0 ${lineChartWidth} ${lineChartHeight}`} preserveAspectRatio="none">
                  <defs>
